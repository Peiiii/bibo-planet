...
当前发布锚点（2026-09-24，北京时间）：ECS 后端应用 Git `2e4ee55`，Cloudflare 前端/Worker 源 Git `829d049`、Worker 版本 `eaa9b684-662b-4fc1-83e7-06f4da2d7f85`。前端、`robots.txt`/`llms.txt` 和受限 CSP 均已在公网；ECS 后端未因这些静态资源修正而重启。公网世界接口含最近相遇时间；服务端会根据明确线索从更早共同遭遇中找回少量片段，不增加模型调用或新持久状态。跨 10 次遭遇的语义由隔离世界中的真实模型试验证明。两个隔离 Chrome 浏览器身份已完成注册、跨访客共享记忆、私人原文隔离、退出重登；手机视口完成真实发消息与回复、历史排版复验，1440×900 桌面首屏可见完整主操作，320px/390px 手机三卡和主操作可见，桌面和手机 Lighthouse 均为四项 100、47 项通过/0 失败。首个自然定时备份与第三轮可靠性审查仍未闭合，不能仅凭本节称为正式获客成品。
