...
3. 确认 `/var/lib/bibo-planet` 有足够磁盘空间、服务内存未接近 `MemoryMax=900M`；观察模型 401/402/429/超时、每日尝试额度与供应商余额/账单。每账号每天最多 18 次业务层唤醒尝试与 12 次成功唤醒，全星球最多 240 次业务层尝试；调用前预留并持久化，失败也占尝试预算。账号层另有每天 240 个成功注册的持久上限、单 IP 与全站短窗口尝试节流，进程内 IP 计数表最多保留 10,000 个键；每账号最多 8 个有效会话，成功注册/登录时清除过期哈希，无效退出不写盘。第 9 个会话会让最早的会话失效，持有它的设备下次访问时需重新登录。达到 429 时先区分注册名额、短窗口节流与模型尝试额度，不擅自通过重启、删除账号或改状态文件绕过。短窗口和内存保护不能替代边缘防护，多实例不会共享当前单机计数。**模型尝试上限不是供应商实际 HTTP 请求数或账单金额的硬上限**：已安装的 NextClaw 兼容适配器在瞬时错误下每个 API base 可重试最多 3 次，根路径配置还可能试 `/v1`。监测真实余额/账单和[官方价格](https://api-docs.deepseek.com/quick_start/pricing/)，不能只看首页 200；必要时以受控测试账号发一条真实消息。
...
2026-09-24 18:18 再对最新 `daily-2026-09-24-180541.haMhEKFD.tar.gz.gpg` 做精确离机对象隔离恢复：OSS 与 ECS 密文 SHA-256 同为 `0c2e943bb82a5198d615d880445e367027251ed7d8f19e36503157cfda9d25df`，15,379 字节；归档包含账号文件和三只精灵状态。以当前代码在受限临时目录加载成功，聚合出 14 个账号、17 条会话、墨里/皮可/塞拉相遇 20/8/6，共 68 条本人会话消息、34 条本人遭遇；未输出个人数据。临时密文与明文目录已精确删除并验证不存在。仍未在生产数据目录原位覆盖恢复，也未观察到首次自然 timer 触发。

## 版本更新与回滚

当前线上纠偏锚点（2026-09-24 21:30，北京时间）：ECS Bibo 后端 Git `469e83b`，Cloudflare Worker `0a3e79e2-63db-4251-a148-ade7683c60bc`。文案和模型回复已去默认虚拟角色扮演，普通身份/能力问题不会注入不相关的旧星球记录；公网真实模型问答和跨访客相关线索各实测一轮，桌面/320px 页面与注册弹窗已检查。更新前加密备份 `daily/daily-2026-09-24-212407.es4oRER1.tar.gz.gpg` 在私有 OSS 存在（18,656 字节），这份尚未做恢复验证；Bibo 和备份 timer active，`NRestarts=0`。**运行链路仍是 NextClaw 模型接口而非真正的 Agent run**，不得将此版本对外表述为 Agent 已交付；相关安全迁移与其他未完成验收见[公网交付记录](plans/2026-09-24-public-launch.plan.md)。
