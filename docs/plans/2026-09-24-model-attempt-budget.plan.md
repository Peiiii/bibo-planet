...
- **成功额度与模型尝试分开**：失败占一次尝试，不虚记成功回复；保留 6 次个人失败缓冲，同时全世界最高 240 次**业务层唤醒尝试**；采用。供应商实际请求数可能因适配器内部重试或备用 API 路径而更高，不能把这个数字写成供应商请求硬上限。
...
本计划提供**业务层模型唤醒尝试次数上限**，不是供应商 HTTP 请求数或账单金额的绝对上限。2026-09-24 对实际安装的 `@nextclaw/core@0.18.3` 产物核查发现，OpenAI 兼容适配器对瞬时错误每个 API base 最多尝试 3 次，根路径配置还可能尝试 `/v1` 备用路径；因此单个预留可对应多个供应商请求。即使供应商账单只按 token 计费，也须持续监测实际用量、余额和[官方价格](https://api-docs.deepseek.com/quick_start/pricing/)。浏览器控制不可用时，只能报告 API 证据，UI 验收保持开放。
