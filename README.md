...
`planet.bibo.bot` 由 Cloudflare Worker 托管静态前端，并将 `/api/*` 通过 HTTPS 代理到受保护的 Node 服务。Node 服务只监听云主机 loopback，要求随机边缘认证头；模型密钥仅在后端运行环境里。账号密码使用 scrypt 哈希，会话 cookie 为 `HttpOnly`、公网 `Secure`，账号、世界与私人对话持久化到独立数据目录。公开服务设有每账号每日唤醒次数、全站上限和注册/登录节流；它们是成本/滥用保护，不是精灵世界的所有权规则。
