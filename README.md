...
AI 会参考近期的共同记录；当你提起足够明确的旧线索时，它也会从更早的对话中找回相关片段。这不是完整、无遗漏的长期记忆。每个人只能在自己的会话列表中查看自己的原始对话，但不要输入不希望被间接转述的秘密。

这不是 NextClaw 私人助手的界面皮肤。Bibo 独立拥有账号、共享记录、额度和前端。**当前公网版本只通过 `@nextclaw/harness` 调用模型，并未运行 NextClaw Agent**，也没有文件、命令或网络工具；这是未完成的临时架构，不应被宣传为最终产品。目标是让三个共享 AI 真正运行 NextClaw Agent；开放工具前必须完成执行隔离，不能用提示词或 `restrictToWorkspace` 冒充沙箱。
